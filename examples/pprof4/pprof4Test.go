...
import (
	...
	sq "github.com/gohutool/boot4go-queue"
)
...
func testQueueHigh(grp, cnt int, sched bool,
	...
	var Qt = newQtSum(grp)
	wg.Add(grp)
	var q *sq.Queue[int] = sq.NewQueue(1024*1024, valCnt)
	for i := 0; i < grp; i++ {
		go func(g int) {
			for j := 0; j < cnt; j++ {
				putCnt := uint32(0)
...
func TestQueueHigh() {
	...
	for i := 1; i <= runtime.NumCPU()*4; i++ {
		...
		fmt.Printf("%v, Grp: %3d, Times: %10d, miss:%6v, use: %12v, %8v/op  %6v/op\n",
			runtime.Version(), i, sum, miss, use, op, int(op)/valCnt)
		Use += use
		Sum += sum
	}
	op := Use / time.Duration(Sum)
	fmt.Printf("%v %v, Grp: %3v, Times: %10d, miss:%6v, use: %12v, %8v/op  %6v/op\n",
		runtime.Version(), runtime.GOARCH, "Sum", Sum, 0, Use, op, int(op)/valCnt)
}
