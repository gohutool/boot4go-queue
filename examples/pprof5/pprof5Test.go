			...
			for j := 0; j < cnt; j++ {
				for d := range valPut {
					...
					for !ok {
						...
						ok, _ = q.Put(valPut[d])
						if miss > 1000 {
							panic(fmt.Sprintf("Put Fail  putCnt:%12v, putMis:%12v, getCnt:%12v, getMis:%12v\n %v \n",
	...
	go func(g int) {
		...
		for j := 0; j < cnt*grp; j++ {
			...
			for getCnt < valCnt {
				...
				getCnt += gets
				if miss > 100 {
					panic(fmt.Sprintf("Get Miss  putCnt:%12v, putMis:%12v,  getCnt:%12v, getMis:%12v, gets:%12v\n %v \n",
