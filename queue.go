...
func NewQueue[T any](capacity uint32, prototype T) *Queue[T] {
	...
	for i := range q.cache {
		var cache *Cache[*T] = &q.cache[i]
		cache.getNo = uint32(i)
		cache.putNo = uint32(i)
	}

	var cache *Cache[*T] = &q.cache[0]
...
func (q *Queue[T]) Capacity() uint32 {
	return q.capacity
}

func (q *Queue[T]) Quantity() uint32 {
...
// Put queue functions
func (q *Queue[T]) Put(val *T) (ok bool, quantity uint32) {
...
func (q *Queue[T]) Puts(values []*T) (puts, quantity uint32) {
	...
	for posNew, v := putPos+1, uint32(0); v < putCnt; posNew, v = posNew+1, v+1 {
		var cache *Cache[*T] = &q.cache[posNew&capMod]
		for {
			getNo := atomic.LoadUint32(&cache.getNo)
			putNo := atomic.LoadUint32(&cache.putNo)
...
// Get queue functions
func (q *Queue[T]) Get() (val *T, ok bool, quantity uint32) {
...
func (q *Queue[T]) Gets(values []*T) (gets, quantity uint32) {
